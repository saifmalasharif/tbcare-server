  ...
  try {
    await pool.query(
      `UPDATE patients SET name=$1, code=$2, age=$3, gender=$4, phone=$5, gov=$6, sector=$7,
       ...
       notes=$21, extensions=$22, extension_log=$23, completed_months=$24,
       treatment_status=$25, treatment_end_date=$26, treatment_outcome=$27,
       updated_at=NOW() WHERE id=$28`,
      [p.name, p.code, p.age, p.gender, p.phone, p.gov, p.sector, p.tbtype,
       p.site, p.category, p.smear, p.center, p.startDate, p.drug1, p.drug2,
       p.daily, p.total, p.remaining, p.lastRefill, p.selfcollect || false, p.notes || '',
       p.extensions || 0, JSON.stringify(p.extensionLog || []), p.completedMonths || 6,
       p.treatmentStatus || null, p.treatmentEndDate || null, p.treatmentOutcome || null,
       req.params.id]
    );
...
app.get('/api/reset-passwords-tbcare2024', async (req, res) => {
  try {
    ...
    for (const [username, password, role, gov, label] of users) {
      ...
      if (ex.rows.length > 0) {
        await pool.query('UPDATE users SET password=$1, role=$2, governorate=$3, label=$4, active=true WHERE username=$5',
          [hash, role, gov, label, username]);
      } else {
        await pool.query('INSERT INTO users (username, password, role, governorate, label) VALUES ($1,$2,$3,$4,$5)',
